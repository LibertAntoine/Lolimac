...
class UserManager extends DBAccess {
	...
	public function add(User $user) {
      ...
      VALUES (:fisrtname, :lastname, :pseudo, :mail, :phone, :pwd_hash, :photo_url, :status, :year_promotion);");
  ...
  public function readByPseudo($pseudo) {
      $q = $this->db->prepare('SELECT * WHERE pseudo = :pseudo');
  ...
  public function update(User $user) {
    $q = $this->db->prepare('UPDATE users SET firstname = :firstname, lastname = :lastname, pseudo = :pseudo, mail = :mail, phone = :phone, pwd_hash = :pwd_hash, photo_url = :photo_url, status = :status WHERE id = :id');
